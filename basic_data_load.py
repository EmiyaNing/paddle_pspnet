...
class BasicDataLoader():
    ...
    def preprocess(self, data, label):
        # this place will add a new axis to label. I don't know why should do that..
        h, w, c     = data.shape
        h_gt, w_gt  = label.shape
        assert h==h_gt, "Error rows"
        assert w==w_gt, "Error cols"
        if self.transform:
...
def main():
    ...
    with fluid.dygraph.guard(place):
        ...
        basic_dataloader = BasicDataLoader(image_folder=image_folder, image_list_file=image_list_file, transform = transform, shuffle=True)
